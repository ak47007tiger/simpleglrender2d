package com.hpl.simpleglrender2d.animation;

/**
 * The Ease class provides a collection of ease functions. It does not use the standard 4 param ease
 * signature. Instead it uses a single param which indicates the current linear ratio (0 to 1) of
 * the tween.
 */
public class EaseProvider {
  ...
  private static float getBackInOut(float elapsedTimeRate, float amount) {
    amount *= 1.525;
    if ((elapsedTimeRate *= 2) < 1) {
      return (float) (0.5 * (elapsedTimeRate * elapsedTimeRate * ((amount + 1) * elapsedTimeRate
          - amount)));
    }
    return (float) (0.5 * (
        (elapsedTimeRate -= 2) * elapsedTimeRate * ((amount + 1) * elapsedTimeRate + amount) + 2));
  }
  ...
  private static float getElasticIn(float elapsedTimeRate, double amplitude, double period) {
    ...
    return (float) -(amplitude * Math.pow(2f, 10f * (elapsedTimeRate -= 1f)) * Math
  ...
  private static float getElasticOut(float elapsedTimeRate, double amplitude, double period) {
    ...
    double pi2 = Math.PI * 2;
    double s = period / pi2 * Math.asin(1 / amplitude);
    return (float) (amplitude * Math.pow(2, -10 * elapsedTimeRate) * Math
        .sin((elapsedTimeRate - s) * pi2 / period) + 1);
  }
  ...
  private static float getElasticInOut(float elapsedTimeRate, double amplitude, double period) {
    ...
    double s = period / pi2 * Math.asin(1 / amplitude);
    if ((elapsedTimeRate *= 2) < 1) {
      return (float) (-0.5f * (amplitude * Math.pow(2, 10 * (elapsedTimeRate -= 1f)) * Math
          .sin((elapsedTimeRate - s) * pi2 / period)));
    }
    return (float) (amplitude * Math.pow(2, -10 * (elapsedTimeRate -= 1)) * Math
